...
public class DrawTool extends Subject {
	...
	public DrawTool(MapView mapView) {
		...
		defaultListener = new MapOnTouchListener(this.mapView.getContext(),this.mapView);
		...
		public boolean onTouch(View view, MotionEvent event) {
			if (active
				...
				case DrawTool.FREEHAND_POLYLINE:
					...
					&& (drawType == ENVELOPE || drawType == CIRCLE)) {
				sendDrawEndEvent();
				startPoint = null;
			}
			...
			if (active
				...
				case DrawTool.ENVELOPE:
					...
					envelope.setYMax(startPoint.getY() < point.getY() ? point
							.getY() : startPoint.getY());
					DrawTool.this.tempLayer.updateGraphic(graphicID,
							envelope.copy());
					break;
				case DrawTool.FREEHAND_POLYGON:
